    ...
    try {
      ...
      const [
        ...
        API.get('/admin/all-instructors').catch(e => {
          ...
          return API.get('/admin/instructors').catch(() => ({ data: { instructors: [] } }));
        }),
        API.get('/admin/courses').catch(() => ({ data: { courses: [], pendingCourses: [] } })),
      ...
      setAllUsers(usersResponse.data.users || usersResponse.data.students || []);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        <div className="text-center">
          <div className="relative">
            ...
            <div className="absolute inset-0 w-16 h-16 border-4 border-transparent border-t-pink-400 rounded-full animate-spin mx-auto my-2" style={{animationDirection: 'reverse', animationDuration: '0.8s'}}></div>
          </div>
          <p className="text-white/70 mt-4 animate-pulse">Loading dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-40 -left-40 w-80 h-80 bg-blue-500/20 rounded-full blur-3xl animate-pulse" style={{animationDelay: '2s'}}></div>
        <div className="absolute bottom-40 right-40 w-60 h-60 bg-pink-500/20 rounded-full blur-3xl animate-pulse" style={{animationDelay: '4s'}}></div>
      </div>
          ...
          <div className="flex justify-between items-center">
            <div className="animate-fade-in-up">
              ...
              <p className="text-white/70 mt-2 text-lg">
                ...
                <span className="block sm:inline"> Manage your Trainify platform.</span>
              </p>
            </div>
              ...
              <div className="backdrop-blur-xl bg-white/10 rounded-3xl p-8 border border-white/20 shadow-2xl hover:shadow-purple-500/10 transition-all duration-500 animate-slide-in-left">
                ...
                <div className="space-y-4 max-h-80 overflow-y-auto custom-scrollbar">
                  {recentActivity.length > 0 ? recentActivity.map((activity, index) => (
                    <div key={index} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-all duration-300 transform hover:scale-[1.02]" style={{animationDelay: `${index * 100}ms`}}>
                      <div className="flex items-center gap-4">
                        <div className={`w-3 h-3 rounded-full ${getActivityColor(activity.type)} animate-pulse`}></div>
                        <div>
                          <p className="text-sm font-medium text-white">{activity.description}</p>
                          <p className="text-xs text-white/50">{activity.timestamp}</p>
                        </div>
                      </div>
                      <span className="text-xs text-white/40 bg-white/5 px-3 py-1 rounded-full">{activity.user}</span>
                    </div>
                  )) : (
                ...
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-white/60">{adminStats.totalUsers} total users</p>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-white/60">{pendingApprovals.length} pending approval</p>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div>
                      <p className="font-semibold text-white">Moderation Queue</p>
                ...
                <table className="min-w-full">
                  ...
                  <tbody className="divide-y divide-white/10">
                    {allUsers.map((user, index) => (
                      <tr key={user._id} className="hover:bg-white/5 transition-all duration-300" style={{animationDelay: `${index * 50}ms`}}>
                        ...
                        <td className="px-8 py-6">
                          <span className={`px-3 py-1 text-sm font-semibold rounded-full flex items-center gap-2 w-fit ${
                            user.isBlocked 
                              ? 'bg-gradient-to-r from-red-500 to-rose-500 text-white shadow-lg shadow-red-500/25'
                              : 'bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg shadow-green-500/25'
                          }`}>
                            <div className={`w-2 h-2 rounded-full ${user.isBlocked ? 'bg-white animate-pulse' : 'bg-white animate-pulse'}`}></div>
          ...
          <div className="backdrop-blur-xl bg-white/10 rounded-3xl border border-white/20 shadow-2xl animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-8">
              {allInstructors.map((instructor, index) => (
                <div key={instructor._id} className="backdrop-blur-sm bg-white/5 p-6 rounded-3xl border border-white/20 hover:bg-white/10 transition-all duration-500 transform hover:scale-[1.02] hover:shadow-xl hover:shadow-green-500/10" style={{animationDelay: `${index * 100}ms`}}>
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-white/60">{instructor.email}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl">
                      ...
                      <span className="font-semibold text-white bg-blue-500/20 px-3 py-1 rounded-full">{instructor.coursesCreated || 0}</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl">
                      <span className="text-white/70">Total Students:</span>
                      <span className="font-semibold text-white bg-purple-500/20 px-3 py-1 rounded-full">{instructor.totalStudents || 0}</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl">
                      <span className="text-white/70">Total Revenue:</span>
                      <span className="font-semibold text-white bg-green-500/20 px-3 py-1 rounded-full">₹{instructor.totalRevenue || 0}</span>
                    </div>
                  </div>
                  
                  <div className="mt-4 pt-4 border-t border-white/20">
                    <span className={`px-4 py-2 text-sm rounded-full font-semibold flex items-center gap-2 w-fit ${
          ...
          <div className="backdrop-blur-xl bg-white/10 rounded-3xl border border-white/20 shadow-2xl animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-8">
              {allCourses.map((course, index) => (
                <div key={course._id} className="backdrop-blur-sm bg-white/5 p-6 rounded-3xl border border-white/20 hover:bg-white/10 transition-all duration-500 transform hover:scale-[1.02] hover:shadow-xl hover:shadow-blue-500/10" style={{animationDelay: `${index * 100}ms`}}>
                  <div className="mb-4">
                    <h3 className="font-bold text-white text-lg mb-2 line-clamp-2">{course.title}</h3>
                    <p className="text-sm text-white/60 line-clamp-3">{course.description}</p>
                  </div>
                  
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl">
                      <span className="text-white/70">Instructor:</span>
                      <span className="font-semibold text-white">{course.createdBy?.name}</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl">
                      <span className="text-white/70">Students:</span>
                      <span className="font-semibold text-white bg-purple-500/20 px-3 py-1 rounded-full">{course.studentsEnrolled?.length || 0}</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl">
                      <span className="text-white/70">Price:</span>
                      <span className="font-semibold text-white bg-green-500/20 px-3 py-1 rounded-full">₹{course.price}</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl">
                      <span className="text-white/70">Category:</span>
                      <span className="font-semibold text-white bg-blue-500/20 px-3 py-1 rounded-full">{course.category}</span>
                    </div>
                  </div>
                  
                  <div className="mt-4 pt-4 border-t border-white/20 flex justify-between items-center">
                    <span className={`px-4 py-2 text-sm rounded-full font-semibold flex items-center gap-2 ${
                      course.isApproved 
                        ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg shadow-green-500/25'
              ...
              {pendingApprovals.length > 0 ? (
                <div className="space-y-6">
                  {pendingApprovals.map((item, index) => (
                    <div key={item._id} className="backdrop-blur-sm bg-white/5 border border-white/20 rounded-3xl p-6 hover:bg-white/10 transition-all duration-500 transform hover:scale-[1.01]" style={{animationDelay: `${index * 100}ms`}}>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-sm">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-white/60">By {item.createdBy?.name}</span>
                            </div>
                ...
                <div className="text-center py-16">
                  ...
                  <h3 className="text-2xl font-bold text-white mb-2">No pending moderation</h3>
                  ...
                  <div className="mt-8 inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 rounded-2xl">
                    ...
                    <span className="text-green-300 font-medium">System Clean</span>
                  </div>
                </div>
              )}
            </div>
...
const StatCard = ({ title, value, icon, gradient, subtitle, delay }) => (
  <div className={`backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-6 shadow-2xl hover:shadow-purple-500/10 transition-all duration-500 transform hover:scale-[1.02] animate-scale-in`} style={{animationDelay: delay}}>
    <div className="flex items-center justify-between">
      <div className="flex-1">
        <div className="flex items-center gap-4 mb-4">
          <div className={`w-14 h-14 bg-gradient-to-r ${gradient} rounded-2xl flex items-center justify-center text-2xl shadow-lg`}>
            {icon}
          </div>
          <div>
...
const getActivityColor = (type) => {
  switch (type) {
    case 'enrollment': return 'bg-green-400';
    case 'course_created': return 'bg-blue-400';
    case 'user_registered': return 'bg-purple-400';
    case 'payment': return 'bg-yellow-400';
